...
export default function AllMovies({ refresh, onEdit = () => { } }) {
    ...
    return (
        <div className="w-full min-h-[60vh] flex flex-col items-center justify-start">
            {loading && (
                <p className="text-yellow-300 text-center animate-pulse text-lg mt-10">
                    A carregar os filmes...
                </p>
            )}

            {error && (
                <p className="bg-red-600 text-white px-4 py-2 rounded text-center mt-10">
                    {error}
                </p>
            )}
